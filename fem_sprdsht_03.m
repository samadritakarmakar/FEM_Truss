#!/usr/bin/octave
clear
xls=xlsopen ('fem_truss.xlsx', 1);
#Entry for 1D 2D or 3D
do
 D=A = xlsread ('fem_truss.xlsx', 'Input', 'B2');
until (D>=1 && D<=3)	#Check entry is between 1 and 3

E=xlsread ('fem_truss.xlsx', 'Input', 'B3');	#Entry for number of elements
N=xlsread ('fem_truss.xlsx', 'Input', 'B4');	#Entry for number of nodes
K=zeros(N*D);	#Intialize global stiffness matrix to zero
e=zeros(E,2);	#Intialize nodes entries to zero

#Entry for node numbers
Input=strcat("E2:F",num2str(E+1));
e=xlsread ('fem_truss.xlsx', 'Input', Input);
#for i=1:E
